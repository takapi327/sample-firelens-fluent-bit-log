  ...
  private def accessLog(start: Long, status: Int)(implicit rh: RequestHeader) = {
    if (!IGNORE_PREFIX_LIST.exists(rh.path.startsWith)) {
      ...
      val data = Json.obj(
        ...
        "client_os_version"   -> client.map(v => Seq(v.os.family, v.os.minor, v.os.patch, v.os.patchMinor).mkString(".")),
        ...
        "client_ua_version"   -> client.map(v => Seq(v.userAgent.major, v.userAgent.minor, v.userAgent.patch).mkString(".")),
